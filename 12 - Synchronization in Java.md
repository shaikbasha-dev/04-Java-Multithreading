...
Synchronization in Java is used to control access to shared resources when multiple threads are running at the same time. It helps prevent problems such as inconsistent results, race conditions, and data corruption.
...
Synchronization means allowing only one thread to access a shared resource at a time. It ensures that the critical section of code is executed safely.

A critical section is a block of code that accesses shared data or resources.

## 2. WHY SYNCHRONIZATION IS NEEDED

Without synchronization, multiple threads may read or modify the same variable at the same time, causing incorrect results.
...
A race condition happens when two or more threads access shared data at the same time and the final result depends on the timing of execution.

**Example:**
If two threads both increase a counter from 5 to 6 at the same moment, the final result may be wrong if the update is not protected.
...
Synchronization in Java is used to make shared resources safe for multiple threads. It prevents race conditions by allowing only one thread to access a critical section at a time. Using synchronized methods or synchronized blocks is essential for writing correct thread-safe programs.
